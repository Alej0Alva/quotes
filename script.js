...
let autoChangeTimer = null; // Nuevo: Variable para el temporizador
...
document.addEventListener('DOMContentLoaded', () => {
    // Cargar últimos datos guardados del localStorage
    ...
    // Intentar cargar el archivo de sentencias predeterminado
    tryLoadDefaultSentences();
    
    // Nuevo: Iniciar el temporizador para cambio automático
    startAutoChangeTimer();
});

// Nuevo: Función para iniciar el temporizador de cambio automático
...
// Intenta cargar el archivo de sentencias predeterminado
function tryLoadDefaultSentences() {
    fetch('sentences.json')
        ...
        .then(data => {
            if (sentences.length === 0 && data && data.sentences && data.sentences.length > 0) {
        ...
        .catch(error => {
            console.log('No se encontró archivo sentences.json predeterminado o está vacío.');
        });
}
...
function displayCurrentSentence() {
    ...
    if (0 <= currentIndex && currentIndex < sentencesToUse.length) {
        ...
        sentenceDisplay.textContent = "No sentences available. Add some sentences or upload a file to begin.";
...
function prevSentence() {
    ...
    // Nuevo: Reiniciar el temporizador cuando el usuario interactúa
    startAutoChangeTimer();
}
...
function randomSentence() {
    ...
    if (unviewed.length) {
        ...
        // Modificado: Si todas han sido vistas, reiniciar y elegir aleatoriamente
        viewedIndices.clear();
        currentIndex = Math.floor(Math.random() * sentencesToUse.length);
        displayCurrentSentence();
    }
    
    // Nuevo: Reiniciar el temporizador cuando cambia la sentencia
    startAutoChangeTimer();
}
...
// Funciones para agregar/editar sentencias
function addSentence() {
    const sentence = sentenceInput.value.trim();
    if (sentence) {
        // Verificar si ya existe esta sentencia
        const isDuplicate = sentences.some(s => s.content.toLowerCase() === sentence.toLowerCase());
        
        if (isDuplicate) {
            const confirm = window.confirm("This sentence already exists. Add it anyway?");
            if (!confirm) return;
        }
        
        sentences.push({ content: sentence, favorite: false });
        sentenceInput.value = '';
        
        // Si hay un filtro activo, actualizar los resultados filtrados
        if (isFilterActive) {
            filterSentences();
        } else {
        ...
        // Nuevo: Reiniciar el temporizador cuando se añade una sentencia
        startAutoChangeTimer();
    }
}
...
function editSentence() {
    ...
    if (newContent !== null && newContent.trim()) {
        ...
        // Nuevo: Reiniciar el temporizador cuando se edita una sentencia
        startAutoChangeTimer();
    }
}
...
function deleteSentence() {
    ...
    if (confirm("Are you sure you want to delete this sentence?")) {
        ...
        // Nuevo: Reiniciar el temporizador cuando se elimina una sentencia
        startAutoChangeTimer();
    }
}
    ...
    reader.onload = function(e) {
        try {
            ...
            if (data && data.sentences && Array.isArray(data.sentences)) {
                // Añadir propiedad favorite si no existe
                sentences = data.sentences.map(sentence => {
                    if (!sentence.hasOwnProperty('favorite')) {
                        return { ...sentence, favorite: false };
                    }
                ...
                // Resetear filtro si está activo
                ...
                alert(`Imported ${sentences.length} sentences successfully.`);
                
                // Nuevo: Reiniciar el temporizador cuando se importan sentencias
                startAutoChangeTimer();
            } else {
                throw new Error("Invalid JSON format. Expected {sentences: [{content: '...'}]}");
            }
        } catch (error) {
...
function exportJSON() {
    ...
    a.download = `sentences_${timestamp}.json`;
    a.click();
...
function filterSentences() {
    ...
    // Nuevo: Reiniciar el temporizador cuando se cambia el filtrado
    startAutoChangeTimer();
}
...
function showAllSentences() {
    ...
    // Nuevo: Reiniciar el temporizador cuando se muestran todas las sentencias
    startAutoChangeTimer();
}
...
function toggleFavoritesFilter() {
    ...
    // Nuevo: Reiniciar el temporizador cuando se cambia el filtro de favoritos
    startAutoChangeTimer();
}
...
function toggleFavorite() {
    ...
    // Nuevo: Reiniciar el temporizador cuando se cambia un favorito
    startAutoChangeTimer();
}
...
function saveToLocalStorage() {
    const data = {
        sentences,
        currentIndex,
        viewedIndices: Array.from(viewedIndices),
        isFilterActive,
        isShowingFavorites
    };
    localStorage.setItem('sentenceVisualizerData', JSON.stringify(data));
}

function loadFromLocalStorage() {
    const saved = localStorage.getItem('sentenceVisualizerData');
    if (saved) {
        try {
            const data = JSON.parse(saved);
            sentences = data.sentences || [];
            currentIndex = data.currentIndex || -1;
            viewedIndices = new Set(data.viewedIndices || []);
            isFilterActive = data.isFilterActive || false;
            isShowingFavorites = data.isShowingFavorites || false;
            ...
            if (isFilterActive) {
                if (isShowingFavorites) {
                    ...
                    // Restablecer filtro ya que no podemos guardar la consulta
                    isFilterActive = false;
                }
            }
            ...
            // Ocultar por defecto los botones de edición
            if (editButtonsPanel) {
                editButtonsPanel.style.display = 'none';
            }
        } catch (error) {
